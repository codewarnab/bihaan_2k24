...
export default function Component({ results, handleScanAgain, user }: { results: Result[], handleScanAgain: () => void, user: IUser | null }) {
        ...
        if (results.length > 0) {
            try {
                ...
                if (parsedResult.id) {
                    ...
                    setIsOpen(true)
                    fetchStatus(parsedResult.id, parsedResult.isVolunteer)
                } else {
    ...
    return (
        <Drawer open={isOpen} onOpenChange={setIsOpen} onClose={handleScanAgain}>
            <DrawerContent className={scanResult?.isLate ? 'bg-red-50' : ''}>
                <DrawerHeader>
                ...
                {scanResult ? (
                    <div className="p-4 space-y-4">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <span>{scanResult.veg_nonveg.toLowerCase() === 'veg' ? 'Vegetarian' : 'Non-Vegetarian'}</span>
                        </div>
